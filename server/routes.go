...
func (s *Server) scheduleRunner(ctx context.Context, name string, caps []Capability, requestOpts map[string]any, keepAlive *api.Duration) (*runnerRef, error) {
	...
	if missing := model.Has(caps...); len(missing) > 0 {
		return nil, fmt.Errorf("model does not support %v", missing)
	}
	...
	// update runtime options and model
	runner.Options = &opts
	runner.model = model
	return runner, nil
}
...
func (s *Server) GenerateHandler(c *gin.Context) {
	...
	if req.Tools != nil {
		caps = append(caps, CapabilityToolCall)
	}
...
func (s *Server) ChatHandler(c *gin.Context) {
	...
	prompt, images, err := chatPrompt(c.Request.Context(), r, req.Messages, req.Tools)
	if err != nil {
		c.JSON(http.StatusInternalServerError, gin.H{"error": err.Error()})
		return
	}
	...
	if req.Stream != nil && !*req.Stream {
		...
		if toolCalls, ok := r.model.tryParseToolCalls(b.String()); ok {
